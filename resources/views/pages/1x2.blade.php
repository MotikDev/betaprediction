...
@section('content')
    ...
    <div style="text-align: center">
        ...
        <!--Form to handle date-->
        <form action="{{ action('PagesController@_1x2') }}" method="POST" role="form">
            <input type="hidden" name="_method" value="GET">

            <div class="form-group">
                <input type="hidden" name="_token" value="{{ csrf_token() }}">
                <input type="date" name="date" value="{{ $mytime }}">
                <input type="submit" class="btn btn-info btn-sm" value="Search">
            </div>
        </form>

    </div> <!--Closing "div" for time-->

    <h3 class="text-center text-primary">1X2 Predictions for Today</h3>
    ...
    <table class="table">
        ...
        <tbody>
            @if (count($posts) > 1)
                ...
                @endforeach
            @else
                <tr colspan="6"><h1 class="text-center">No prediction found for this day.</h1></tr>
            @endif
        </tbody>
    </table>
...
@section ('sidebar')
    <br> 
    <h4>This is where my advert is going to be positioned.</h4>
@endsection
                            ...
                            function {{ $reply }}()
                            {
                                if((count%2) == 1)
                                {
                                    ...
                                    return document.getElementById("{{ $reply }}").innerHTML = 
                                                ...
                                                +'<input type="hidden" name="_token" value="{{ csrf_token() }}">'
                                                +'<input type="hidden" name="pageName" value="1x2">'
